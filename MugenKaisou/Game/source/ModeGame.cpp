...
// 時間変更1000秒
class MenuItemTime : public MenuItemBase {
public:
	MenuItemTime(void* param, std::string text) : MenuItemBase(param, text) {}

	// 項目を決定したらこの関数が呼ばれる
	// return int : 0 = メニュー継続, 1 = メニュー終了
	virtual int Selected() {
		ModeGame* mdGame = static_cast<ModeGame*>(_param);
		mdGame->SetDebugTime(mdGame->GetDebugTime());
		return 1;
	}
};
// 部屋スキップ
class MenuItemSkip : public MenuItemBase {
public:
	MenuItemSkip(void* param, std::string text) : MenuItemBase(param, text) {}

	// 項目を決定したらこの関数が呼ばれる
	// return int : 0 = メニュー継続, 1 = メニュー終了
	virtual int Selected() {
		ModeGame* mdGame = static_cast<ModeGame*>(_param);
		mdGame->SetDebugSkip(mdGame->GetDebugSkip());
		return 1;
	}
};
...
#endif
	//gGlobal._vEffekseer[0].AddEffect("res/Laser01.efkefc", 0.0f, 500.0f, 0.0f);
	// テクスチャレンダリング用の画像作成
...
bool ModeGame::Process() {
	...
	if (!rData.IsLoad()) {
		...
		if (gGlobal._isFinishFadeIn) {
			// 部屋切り替わり時、フェードイン終了後にキャラ操作を可能にする
			_flgPlay = true;
		}
		...
		if(_flgPlay) {
			...
			{
				// カメラターゲット（注視点）の更新
				_cam._vTarget = VAdd(_chara._vPos, VGet(0.f, 160.f, 0.f));
				
				// Y軸回転
				float length = sqrt(sz * sz + sx * sx);
				if (rx > analogMin) { camrad -= 0.05f; }
				if (rx < -analogMin) { camrad += 0.05f; }
				_cam._vPos.x = _cam._vTarget.x + cos(camrad) * length;
				_cam._vPos.z = _cam._vTarget.z + sin(camrad) * length;
		...
		if (_chara._vPos.y < -400 || _chara._isDead)
		{
			if (_flgPlay) {
				// フェードアウト
				ModeServer::GetInstance()->Add(new ModeFade("OUTIN", false), 900, "fade");
				_flgPlay = false;
				respown_flg = true;
				gChara_Dead = true;
			}
		...
		if (keyRoom != 1225)
		{
			if (VSize(v) > 0.f) {		// 移動していない時は無視するため
				if (GetModeCount() % 11 == 0 && _chara._isLanded == true) {
					gGlobal._effekseer.AddEffect("res/Effect/dash/dash.efkefc", _chara._vPos.x, _chara._vPos.y, _chara._vPos.z, 60);
			...
			for (int i = 0; i < gGlobal._vEffekseer.size(); i++) {
				...
				// 消去処理
				if (gGlobal._vEffekseer[i].EffectCnt > gGlobal._vEffekseer[i].EffectCntMax && gGlobal._vEffekseer[i].EffectCntMax != -1) {
		...
		if (_chara._flgExit) {
			if (_flgPlay) {
				// フェードインアウト
				ModeServer::GetInstance()->Add(new ModeFade("OUT", false), 998, "fade");
				_flgPlay = false;
			}
			else {
				if (gGlobal._isFinishFadeOut) {
		...
		// デバッグ
		// ------------------------------------------------------------------------
		// ESCキーとDeleteキーの同時押しでデバッグメニューを開く
		if (!gGlobal._flgDebug &&
			(gKeyMouse.gKKey[KEY_INPUT_ESCAPE] && gKeyMouse.gKKey[KEY_INPUT_DELETE])) {
			gGlobal._flgDebug = true;
			
			ModeMenu* modeMenu = new ModeMenu();
			// ModeGameより上のレイヤーにメニューを登録する
			ModeServer::GetInstance()->Add(modeMenu, 800, "debugmenu");
			...
			modeMenu->Add(new MenuItemTime(this, "DegubTime"));
			modeMenu->Add(new MenuItemSkip(this, "SkipRoom"));
		}
	}
...
bool ModeGame::Render() {
	...
	if (!rData.IsLoad()) {
		...
		if(_bViewInfo) {
			...
			DrawFormatString(x, y, GetColor(255, 0, 0), "  campos   = (%5.2f, %5.2f, %5.2f)", _cam._vPos.x, _cam._vPos.y, _cam._vPos.z); y += size;
			DrawFormatString(x, y, GetColor(255, 0, 0), "  target   = (%5.2f, %5.2f, %5.2f)", _cam._vTarget.x, _cam._vTarget.y, _cam._vTarget.z); y += size;
			DrawFormatString(x, y, GetColor(255, 0, 0), "  charapos = (%5.2f, %5.2f, %5.2f)", _chara._vPos.x, _chara._vPos.y, _chara._vPos.z); y += size;
			float sx = _cam._vPos.x - _cam._vTarget.x;
			float sz = _cam._vPos.z - _cam._vTarget.z;
			float length = sqrt(sz * sz + sx * sx);
			...
			// キャラ速度
			DrawFormatString(x, y, GetColor(255, 0, 0), "  v : %5.2f", VSize(_chara._vDir)); y += size;
			// カプセルと衝突したポリゴン数
			...
			// 部屋番号
			DrawFormatString(x, y, GetColor(255, 0, 0), "  Key Now : %d, Next : %d, Lose : %d", keyRoom, gGlobal.keyNextRoom, gGlobal.keyLoseRoom); y += size;
			...
			DrawFormatString(x, y, GetColor(255, 0, 0), "gGlobal._vEffekseer.size():%d", sizeEffect); y += size;
		}
