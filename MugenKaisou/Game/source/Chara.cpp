...
void Chara::Jump() {
	if (jump_flg == 0) {
		if (_isLanded) {
			// 着地状態かつジャンプしていない状態の時、ジャンプ可能
			if (_mapBoxType != 0 && _isJump) {
				if(gPad[0].gXTrg[XINPUT_BUTTON_B]) {
					jump_flg = 1;
					_jumpMove = 20.f;
					_isLanded = false;

					// ジャンプエフェクト
					gGlobal._effekseer.AddEffect("res/Effect/jump/jump.efkefc", _vPos.x, _vPos.y, _vPos.z, 30);
