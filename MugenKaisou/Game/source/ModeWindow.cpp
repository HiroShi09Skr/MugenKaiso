...
bool ModeWindow::Initialize() {
	...
	cgAns[2] = LoadGraph("res/Window/LabyrinthPlus/maru_999.png");
	...
	cglabel[10] = LoadGraph("res/Window/_ScreenTransition/label/1225.png");
	cglabel[11] = LoadGraph("res/Window/_ScreenTransition/label/enter_num.png");
	...
	se[3] = LoadSoundMem("res/sound/SE/propeller.mp3");
	...
	for (i = 0; i < 9; i++) {
		...
		_puzzle[i].x = 1920 / 2 + 30 + (rand() % (1920 / 2 - _puzzle[i].w - 30));
		_puzzle[i].y = 65 + 30 + (rand() % (720 - 65 - _puzzle[i].h - 30));
...
bool ModeWindow::Process() {
	...
	if (gGlobal._isFinishFadeIn) { return true; }
...
bool ModeWindow::Render() {
	...
	//石板
	{
		...
		{
			if (pagenum == 0) {
				DrawGraph(1920 / 2 + 59, 15, cglabel[11], TRUE);
			}
			else
			{
				DrawGraph(1920 / 2 + 59, 24, cglabel[11], TRUE);
			}
			...
			for (int n = 0; n < pageMax; n++) {
				...
				if (_codesave[pagenum - 1].codesave == _codesave[n].codesave) {
					DrawGraph(1920 / 2 + 151 + n * 70, 15, cglabel[DrawLabel], TRUE);
				}
				else
				{
		...
		else
		{
			...
			for (int n = 0; n < jsonMax; n++) {
				if (_jsonData[n].code == codesave) {
					...
					if (_jsonData[6].code == codesave)
					{
						...
						else
						{
							if (QP_step > 3)
							{
								...
								//正解
								if (QP_correct[0] == 1) { DrawGraph(1920 / 2 + 739, 374 + 147, cgAns[2], TRUE); }
							...
							else
							{
								...
								for (int i = 0; i < 4; i++)
								{
									...
									//正解
									if (QP_correct[i] == 1) { DrawGraph(1920 / 2 + 739, 379 + 147 * i, cgAns[1], TRUE); }
									else if (QP_correct[i] == 2) { DrawGraph(1920 / 2 + 739, 379 + 147 * i, cgAns[0], TRUE); }
								}
					...
					if (_jsonData[8].code == codesave) {}
					if (_jsonData[9].code == codesave) {}
					if (_jsonData[10].code == codesave) {}
					DrawGraph(1920 / 2, 0, cgBase[2], TRUE);
					break;
				}
			}
