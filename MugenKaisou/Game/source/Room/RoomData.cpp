		...
		for (int s = 0; s < _mapSteps; s++) {
			for (int i = 0; i < _mapSize; i++) {
				...
				if (data._mapType != 0) {
					...
					// 位置情報
					data._vPos = VGet(static_cast<float>((i % _mapW) * _boxSize), static_cast<float>(s * _boxSize), static_cast<float>(- n * _boxSize));
					// モデル回転
					...
					MV1SetFrameVisible(data._handle, data._frameCollision, _isViewFrameCollision);
