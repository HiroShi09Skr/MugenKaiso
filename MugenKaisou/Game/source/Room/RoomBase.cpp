...
void RoomBase::SetSpown(Chara& chara, Camera& cam) {
	for (int s = 0; s < rData._mapSteps; s++) {
		for (int i = 0; i < rData._mapSize; i++) {
			...
			if (mapType == 6) {
				// キャラ
				chara._vPos = VGet(static_cast<float>((i % rData._mapW) * rData._boxSize), 600.f, static_cast<float>(- n * rData._boxSize));
				chara._vResPos = VGet(static_cast<float>((i % rData._mapW) * rData._boxSize), 600.f, static_cast<float>(- n * rData._boxSize));
				// カメラ
...
void RoomBase::Process(Chara& chara, VECTOR& v, VECTOR oldv) {
