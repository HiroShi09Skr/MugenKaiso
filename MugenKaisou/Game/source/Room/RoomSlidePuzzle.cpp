...
void RoomSlidePuzzle::SetSpown(Chara& chara, Camera& cam) {
	RoomBase::SetSpown(chara, cam);

	// カメラ
	cam._vPos = VGet(chara._vPos.x, chara._vPos.y + 1700.f, chara._vPos.z - 800.f);
	cam._oldvPos = cam._vPos;
}

void RoomSlidePuzzle::Process(Chara& chara, VECTOR& v, VECTOR oldv) {
	if (keyRoom == 1109 && SaveRoot[1] == 0 && gGlobal._isFinishFadeIn) {
		SaveRoot[1] = 1;
		...
		gGlobal._effekseer.AddEffect("res/Effect/doa/doa.efkefc", 3000.0f, 250.0f, -750.0f, -1, 0.0f, 50.0f);
		...
		if (!TalkFlg[19]) {
			...
			ModeServer::GetInstance()->Add(modetalk, 200, "talk");
			TalkFlg[19] = TRUE;
			gGlobal._effekseer.AddEffect("res/Effect/doa/doa.efkefc", 750.0f, 250.0f, -2500.0f, -1, 1.6f, 50.0f);
			gGlobal._vEffekseer.push_back(gGlobal._effekseer);
		}
	...
	for (int i = 0; i < rData._objData.size(); i++) {
		...
		if (chara.isHitWall(rData._objData[i]._handle, rData._objData[i]._frameCollision, true)) {
			...
			if (rData._objData[i]._objType != 1) {
				int newHitNormal = static_cast<int>(chara._vNormal.size());
...
void RoomSlidePuzzle::CameraProcess(Camera& cam) {
	...
	// マップオブジェクト
	for (int i = 0; i < rData._objData.size(); i++) {
		if (rData._objData[i]._objType == 18 || rData._objData[i]._objType == 30) {
