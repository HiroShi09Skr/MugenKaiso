...
void RoomPizza::Process(Chara& chara, VECTOR& v, VECTOR oldv) {
	if (SaveRoot[3] == 0)
	{
		SaveRoot[3] = 1;
		...
		gGlobal._effekseer.AddEffect("res/Effect/doa/doa.efkefc", 3500.0f, 250.0f, -1000.0f, -1, 0.0f, 50.0f);
	...
	if (_vPizza.size() > 0) {
		for (int i = 0; i < _vPizza.size(); i++) {
			...
			if (_vPizza[i]._mapBoxType != 0 && _vPizza[i]._mapBoxType != 1 && _vPizza[i]._mapBoxType != 10) {
				...
				beltConv.MovePos(_vPizza[i]._vPos, _vPizza[i]._mapBoxType, 32.0f);
			}
